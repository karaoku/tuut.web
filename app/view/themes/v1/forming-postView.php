<?php require view('static/header') ?>
    <sub-title class="sub-title">
        <div class="container">
            <h1>TuuT Oluştur</h1>
            <p>TuuT oluştururken uyarıları lütfen dikkate al.</p>
        </div><!-- Container -->
    </sub-title>
    <sub-header class="sub-header">
        <div class="container">
            <?php if (isset($error_forming_tuut)):?>
                <div class="text-box"><?= $error_forming_tuut ?></div>
            <?php endif; ?>
        </div><!-- Container -->
    </sub-header>
    <main class="main" style="margin-top: -2rem">
        <div class="container">
            ...
            <form method="post" enctype="multipart/form-data"style="background: #eee;padding: 2rem;margin-top: -25px;">
            <div class="input-group">
                <label>TuuT Başlığı<div class="notice">[Zorunlu olmayan alan]</div></label>
                <input type="text" max="100" name="tuut_title">
                <label>TuuT Açıklaması<div class="notice">[Zorunlu olmayan alan]</div></label>
                <input type="text" max="300" name="tuut_desc">
                <label>TuuT Kategorisi<div class="notice">[Zorunlu olan alan]</div></label>
                <select name="tuut_category">
                    <?php foreach (Feed::Categories() as $category): ?>
                    <option value="<?=$category['category_id']?>"><?=$category['category_title']?></option>
                    <?php endforeach ?>
                </select>
                <label>TuuT Görseli<div class="notice">[Zorunlu olan alan]</div></label>
                <input type="file" name="tuut_file" accept="image/gif, image/jpeg, image/png">
            </div><!-- İnput-Group-->
            <input type="hidden" name="submit-forming-tuut" value="1">
            <button type="submit" class="btn-dark">Paylaş</button>
            </form>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
